...
Для запуска приложения необходимо сконфигурировать следующие переменные окружения:
...
- `JWT_SECRET` - секретный ключ для подписи JWT токенов. Должен быть достаточно длинным и сложным.
